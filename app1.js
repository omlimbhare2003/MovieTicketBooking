...
async function renderMovies(){
    ...
    //to temp show loading spinner untill movie arrive
...
function setEventToLinks(){
    ...
    movieLinks.forEach(movieLink => {
        movieLink.addEventListener('click', (e) => {
            // console.log(movieLink.getAttribute('data-movie-name'));
...
async function renderSeatsGrid(movieName){
    ...
    //data will be array availble seats for that movie;
    renderSeats(data);
    setEventsToSeats();
}
...
document.getElementById('aboutPage').addEventListener('click',function(){
    overlay.classList.remove('v-none');
    openAboutModal();
})

knowMore.addEventListener('click',function(){
    overlay.classList.remove('v-none');
    modal.classList.remove('v-none');
    modalPara.innerHTML = `
    Movie Point : Made with Your love for movies.
    Watch Latest Movies in your City at best price.
    <br />
...
function openAboutModal(){
    modal.classList.remove('v-none');
    modalPara.innerHTML = `Welcome To Movie Booking App. We are commited to provide you best theatre experience. Book from use to get latest movies at discounted price.
    Service available at all major cities.
    <br />
    For More Info Contact Us @
    <br />
    <h4>VIIT Pune</h4>
...
function closeModal(){
    overlay.classList.add('v-none');
    modal.classList.add('v-none')
}